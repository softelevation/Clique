...
import {
  View, Text, SafeAreaView, StyleSheet, StatusBar, TouchableOpacity, Image, FlatList, LogBox, Platform, TextInput,
  Alert, PermissionsAndroid, ScrollView, Linking, Switch
} from 'react-native';
...
export default class EditProfile extends Component {
    ...
    try {
      ...
      if (value !== null) {
        ...
        this.setState({
          ...
          switchResumeURL: user.resume_link_status != null ? user.resume_link_status : 0,

          txtResumeFile: user.resume_file != null ? dictResume : null,
          switchResumeFile: user.resume_file_status != null ? user.resume_file_status : 0,
  ...
  API_SOCIAL_DELETE(isload, item, index) {
    ...
    Webservice.post(APIURL.socialDelete, {
      ...
      .then(response => {
        ...
        console.log('Get Social Delete Response : ' + JSON.stringify(response.data))
  ...
  createData() {
    ...
    let websiteLinks = this.state.ArrWebsite.filter(el => (
      el.media_type == 'website'
    ...
    let socialMailLinks = this.state.ArrSocialMails.filter(el => (
      el.media_type === 'socialMail'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let instagramLinks = this.state.ArrInstagram.filter(el => (
      el.media_type === 'instagram'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let facebookLinks = this.state.ArrFacebook.filter(el => (
      el.media_type === 'facebook'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let twitterLinks = this.state.ArrTwitter.filter(el => (
      el.media_type === 'twitter'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let youtubeLinks = this.state.ArrYoutube.filter(el => (
      el.media_type === 'youtube'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let linkdinLinks = this.state.ArrLinkdIn.filter(el => (
      el.media_type === 'linkdin'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let homeNumberLinks = this.state.ArrHomeNo.filter(el => (
      el.media_type === 'homeNumber'
    ...
    let workNumberLinks = this.state.ArrWorkNo.filter(el => (
      el.media_type === 'workNumber'
    )).map((function ({ id, media_value, country_code, status }) {
      return { id, mediaValue: country_code + "-" + media_value, status };
    }))
    ...
    let otherNumberLinks = this.state.ArrOtherNo.filter(el => (
      el.media_type === 'otherNumber'
    )).map((function ({ id, media_value, country_code, status }) {
      return { id, mediaValue: country_code + "-" + media_value, status };
    }))
    ...
    let musicLinks = this.state.ArrMusic.filter(el => (
      el.media_type === 'music'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let paymentLinks = this.state.ArrPayment.filter(el => (
      el.media_type === 'payment'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
    ...
    let externalLinks = this.state.ArrExternalLink.filter(el => (
      el.media_type === 'externalLink'
    )).map((function ({ id, media_value, status }) {
      return { id, mediaValue: media_value, status };
    }))
        ...
        <SafeAreaView style={styles.container}>
          ...
          <View style={{ flex: 1, backgroundColor: CommonColors.primaryColor }}>
            ...
            <View style={styles.headerView}>
              ...
              <TouchableOpacity style={{ marginRight: 15, alignItems: 'center', justifyContent: 'center', height: 40, width: 40, }}
                onPress={() => this.btnDoneTap()}>
                <Image style={{ width: 25, height: 25, resizeMode: 'contain', tintColor: CommonColors.arrowColor }}
                  source={TickIcon} />
              </TouchableOpacity>
            </View>
              ...
              {this.state.userData != null ?
                <ScrollView style={{ flex: 1, }} bounces={false}>
                  ...
                  {this.state.ProfileImgData != null ?
                    <View style={{
                      marginTop: 20, alignSelf: 'center', width: 150, height: 150, borderRadius: 75,
                      backgroundColor: CommonColors.whiteColor, shadowColor: CommonColors.blackColor,
                      ...
                      <FastImage style={{ width: 150, height: 150, borderRadius: 75 }}
                        source={{ uri: this.state.ProfileImgData.uri }}
                        resizeMode={FastImage.resizeMode.cover}
                      />

                      <TouchableOpacity style={{
                        position: 'absolute', height: 40, width: 40, backgroundColor: CommonColors.whiteColor, borderRadius: 20,
                        alignSelf: 'flex-end', alignItems: 'center', justifyContent: 'center'
                      }}
                        onPress={() => this.btnSelectImage()}>
                        <Image style={{ height: 20, width: 20, resizeMode: 'contain', tintColor: CommonColors.primaryColor }}
                          source={PencilIcon} />
                      </TouchableOpacity>
                    </View>
                    : null}
                  ...
                  <Text style={{ marginLeft: 20, marginRight: 20, marginTop: 20, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                    Privacy
                </Text>
                  <View style={{ marginLeft: 5, marginRight: 5, flexDirection: 'row', alignItems: 'center', padding: 15 }}>
                    <View style={{
                      flex: 1, marginRight: 15, height: 50, flexDirection: 'row',
                      borderRadius: 15, alignItems: 'center', backgroundColor: CommonColors.appBarColor,
                    }}>
                      <Image style={{ marginLeft: 10, height: 30, width: 30, resizeMode: 'contain' }}
                        source={PrivacyIcon}
                      />
                      <Text style={{ marginLeft: 10, color: CommonColors.whiteColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR }}>
                        Private Profile
                    </Text>
                    </View>
                    ...
                    <Switch
                      ...
                      onValueChange={(value) => this.setState({ switchPrivateProfile: value == true ? 1 : 0 })}
                      value={this.state.switchPrivateProfile == 1 ? true : false} />
                  </View>



                  <Text style={{ marginLeft: 20, marginRight: 20, marginTop: 5, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                    Your Mobile Number
                </Text>
                  <View style={{
                    marginLeft: 20, marginRight: 20, marginTop: 15, height: 50, backgroundColor: CommonColors.appBarColor, flexDirection: 'row',
                  ...
                  <View style={{
                    marginBottom: 20,
                    marginLeft: 20, marginRight: 20, marginTop: 10, height: 50, backgroundColor: CommonColors.appBarColor, flexDirection: 'row',
                    borderRadius: 15, alignItems: 'center',
                  }}>
                    <Image style={{ marginLeft: 10, height: 30, width: 30, resizeMode: 'contain' }}
                  ...
                  <Text style={{ marginLeft: 20, color: CommonColors.whiteColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR }}>
                    Your Name
                </Text>
                  ...
                  <TextInput
                    ...
                    style={{
                      marginLeft: 20, marginRight: 20, flex: 1, height: 50, color: CommonColors.whiteColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR,
                      backgroundColor: CommonColors.appBarColor, borderRadius: 10, paddingLeft: 10, paddingRight: 10, marginTop: 10
                    }}
                  ...
                  <Text style={{ marginLeft: 20, color: CommonColors.whiteColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR, marginTop: 20 }}>
                    Bio
                </Text>
                  ...
                  <TextInput
                    ...
                    maxLength={280}
                    style={{
                      marginLeft: 20, marginRight: 20, flex: 1, marginTop: 10, color: CommonColors.whiteColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR,
                      backgroundColor: CommonColors.appBarColor, borderRadius: 10, padding: 10, minHeight: 100
                    }}
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15, marginLeft: 20, marginRight: 20 }}>
                    <Image source={BusinessIcon} />
                    <Text style={{ marginLeft: 10, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                      Job Position
                          </Text>
                  </View>
                  ...
                  {this.state.ArrCompany.length != 0 ?
                    <View >
                      ...
                      <View style={{ marginTop: 10, }}>
                        <FlatList data={this.state.ArrCompany}
                          keyExtractor={item => item.itemId}
                          renderItem={({ item, index }) => (

                            (item.job_position != '' ?
                              <TouchableOpacity style={{
                                marginTop: 5, marginBottom: 10, marginLeft: 20, marginRight: 20, borderRadius: 15, padding: 15,
                                alignItems: 'center', flexDirection: 'row', backgroundColor: CommonColors.appBarColor,
                              }}
                                onPress={() => this.props.navigation.navigate('AddJob', { company_data: JSON.stringify(item), updateData: this.updateData.bind(this) })}>

                                <Image style={{ height: 50, width: 50, borderRadius: 25 }}
                                  source={{ uri: String(this.state.userData.asset_url + item.logo) }} />

                                <View style={{ flex: 1, justifyContent: 'center' }}>
                                  <Text style={{ flex: 1, marginLeft: 10, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_BOLD }}
                                    numberOfLines={1}>
                                    {item.job_position}
                                  </Text>
                                  <Text style={{ flex: 1, marginLeft: 10, fontSize: SetFontSize.ts14, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}
                                    numberOfLines={1}>
                                    {item.name}
                                  </Text>
                                </View>
                                ...
                                <Image style={{ height: 15, width: 15, resizeMode: 'contain', tintColor: CommonColors.whiteColor }}
                  ...
                  <TouchableOpacity style={{
                    marginTop: 5, marginLeft: 20, marginRight: 20, flexDirection: 'row', height: 50, borderRadius: 15,
                    backgroundColor: CommonColors.appBarColor, alignItems: 'center'
                  }}
                    onPress={() => this.props.navigation.navigate('AddJob', { company_data: null, updateData: this.updateData.bind(this) })}>

                    <Image style={{ height: 25, width: 25, resizeMode: 'contain', marginLeft: 10 }}
                      source={AddIcon}
                    />
                    <Text style={{ marginLeft: 10, marginRight: 10, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                      Add New
                      </Text>
                  </TouchableOpacity>
                  ...
                  <View style={{ marginLeft: 5, marginRight: 5, marginTop: 5, flexDirection: 'row', alignItems: 'center', padding: 15, }}>
                    <TouchableOpacity style={{
                      flex: 1, marginRight: 10, height: 50, backgroundColor: CommonColors.appBarColor, flexDirection: 'row',
                      alignItems: 'center', borderRadius: 10
                    }}
                      onPress={() => this.btnChooseResumeFile()}>
                      <Image style={{ marginLeft: 10, height: 30, width: 30, tintColor: CommonColors.arrowColor }}
                        source={AddIcon}
                      />
                      <Text style={{ flex: 1, marginLeft: 10, color: CommonColors.whiteColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR }}
                        numberOfLines={1}>
                        {this.state.txtResumeFile == null ? 'Choose File' : this.state.txtResumeFile.name}
                      </Text>
                    </TouchableOpacity>

                    <Switch
                      onValueChange={(value) => this.setState({ switchResumeFile: value == true ? 1 : 0 })}
                  ...
                  <View style={{ marginLeft: 20, marginRight: 20, flexDirection: 'row', alignItems: 'center', }}>
                    <View style={{
                      ...
                      <Image style={{ marginLeft: 10, height: 30, width: 30 }}
                        source={EditIcon}
                      />
                    ...
                    <Switch
                      onValueChange={(value) => this.setState({ switchResumeURL: value == true ? 1 : 0 })}
                  ...
                  {this.state.ArrHomeNo.length != 0 ?
                    <View style={{ marginTop: 5, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrHomeNo}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        ...
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10, }}>
                            ...
                            <View style={{
                              flex: 1, backgroundColor: CommonColors.appBarColor, borderRadius: 15, alignItems: 'center',
                              marginRight: 10, height: 50, fontFamily: ConstantKeys.Averta_REGULAR, fontSize: SetFontSize.ts16,
                              flexDirection: 'row'
                            }}>
                              ...
                              <TextInput
                                ...
                                onChangeText={(text) => {
                                  ...
                                  var newData = this.state.ArrHomeNo
                                  newData[index].media_value = text.replace(/[^0-9]/g, '')
                  ...
                  {this.state.ArrWorkNo.length != 0 ?
                    <View style={{ marginTop: 5, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrWorkNo}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                  ...
                  {this.state.ArrOtherNo.length != 0 ?
                    <View style={{ marginTop: 5, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrOtherNo}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 20, marginLeft: 20, marginRight: 20 }}>
                    <Text style={{ fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                      Website
                    </Text>
                  </View>
                  ...
                  {this.state.ArrWebsite.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrWebsite}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            <View style={{
                              height: 50, alignItems: 'center', flexDirection: 'row', flex: 1, marginRight: 10,
                              backgroundColor: CommonColors.appBarColor, borderRadius: 15, marginBottom: 10,
                            }}>


                              <Image style={{ height: 30, width: 30, borderRadius: 15, marginLeft: 10, resizeMode: 'contain' }}
                                source={WebIcon} />
                            ...
                            {this.state.ArrWebsite.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteWebsite(item, index)}>
                  ...
                  <TouchableOpacity style={{
                    marginLeft: 20, marginRight: 20, flexDirection: 'row', backgroundColor: CommonColors.appBarColor,
                    borderRadius: 10, height: 50, borderRadius: 15, alignItems: 'center'
                  }}
                    onPress={() => {
                    ...
                    <Image style={{ height: 30, width: 30, resizeMode: 'contain', marginLeft: 10 }}
                      source={AddIcon}
                    />
                    <Text style={{ marginLeft: 10, marginRight: 10, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                      Add Website
                  ...
                  {this.state.ArrSocialMails.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrSocialMails}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            <View style={{
                              height: 50, alignItems: 'center', flexDirection: 'row', flex: 1, marginRight: 10,
                              backgroundColor: CommonColors.appBarColor, borderRadius: 10, marginBottom: 10,
                            }}>

                              <Image style={{ height: 30, width: 30, borderRadius: 20, marginLeft: 10, resizeMode: 'contain' }}
                                source={SocialMailIcon} />
                            ...
                            {this.state.ArrSocialMails.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteEmail(item, index)}>
                                <Image style={{ width: 30, height: 30, resizeMode: 'contain' }}
                                  source={RemoveIcon}
                                />
                              </TouchableOpacity>
                  ...
                  <TouchableOpacity style={{
                    marginLeft: 20, marginRight: 20, flexDirection: 'row', height: 50, borderRadius: 10, alignItems: 'center',
                    backgroundColor: CommonColors.appBarColor
                  }}
                    onPress={() => {
                  ...
                  {this.state.ArrInstagram.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrInstagram}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            {this.state.ArrInstagram.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteInstagram(item, index)}>
                                <Image style={{ width: 30, height: 30, resizeMode: 'contain' }}
                                  source={RemoveIcon}
                                />
                              </TouchableOpacity>
                  ...
                  <TouchableOpacity style={{ marginLeft: 20, marginRight: 20, flexDirection: 'row', height: 50, borderRadius: 10, backgroundColor: CommonColors.appBarColor, alignItems: 'center' }}
                    onPress={() => {
                  ...
                  {this.state.ArrFacebook.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrFacebook}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            {this.state.ArrFacebook.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteFacebook(item, index)}>
                                <Image style={{ width: 30, height: 30, resizeMode: 'contain' }}
                                  source={RemoveIcon}
                                />
                              </TouchableOpacity>
                  ...
                  {this.state.ArrLinkdIn.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrLinkdIn}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            <View style={{
                              height: 50, alignItems: 'center', flexDirection: 'row', flex: 1, marginRight: 10,
                              backgroundColor: CommonColors.whiteColor, borderRadius: 10, marginBottom: 10, backgroundColor: CommonColors.appBarColor
                            }}>
                  ...
                  {/**********  Twitter Mails **********/}
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10, marginLeft: 20, marginRight: 20 }}>
                    <Text style={{ fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}>
                      Twitter
                    </Text>
                  </View>
                  ...
                  {this.state.ArrTwitter.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrTwitter}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            {this.state.ArrTwitter.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteTwitter(item, index)}>
                                <Image style={{ width: 30, height: 30, resizeMode: 'contain' }}
                                  source={RemoveIcon}
                                />
                              </TouchableOpacity>
                  ...
                  {this.state.ArrYoutube.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrYoutube}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            <View style={{
                              height: 50, alignItems: 'center', flexDirection: 'row', flex: 1, marginRight: 10,
                              backgroundColor: CommonColors.whiteColor, borderRadius: 15, marginBottom: 10, backgroundColor: CommonColors.appBarColor
                            }}>
                            ...
                            {this.state.ArrYoutube.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteYoutube(item, index)}>
                                <Image style={{ width: 30, height: 30, resizeMode: 'contain' }}
                                  source={RemoveIcon}
                                />
                              </TouchableOpacity>
                  ...
                  {this.state.ArrMusic.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrMusic}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            {this.state.ArrMusic.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteMusic(item, index)}>
                  ...
                  {this.state.ArrPayment.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrPayment}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            {this.state.ArrPayment.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeletePayment(item, index)}>
                  ...
                  {this.state.ArrExternalLink.length != 0 ?
                    <View style={{ marginTop: 15, marginLeft: 20, marginRight: 20, }}>
                      <FlatList data={this.state.ArrExternalLink}
                        keyExtractor={item => item.itemId}
                        scrollEnabled={false}
                        extraData={this.state}
                        renderItem={({ item, index }) => (
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', }} >
                            ...
                            {this.state.ArrExternalLink.length != 1 ?
                              <TouchableOpacity style={{ width: 30, height: 30, marginLeft: 5 }}
                                onPress={() => this.btnDeleteExLinks(item, index)}>
                                <Image style={{ width: 30, height: 30, resizeMode: 'contain' }}
                                  source={RemoveIcon}
                                />
                              </TouchableOpacity>
                  ...
                  <TouchableOpacity style={{
                    marginLeft: 20, marginRight: 20, marginBottom: 20, flexDirection: 'row', height: 50, borderRadius: 15,
                    backgroundColor: CommonColors.appBarColor, alignItems: 'center'
                  }}
                    onPress={() => {
...
const styles = StyleSheet.create({
  ...
  headerText: {
    flex: 1, fontFamily: ConstantKeys.Averta_BOLD, fontSize: SetFontSize.ts16,
    color: CommonColors.whiteColor, textAlign: 'center'
  },
  viewCountrystyle: {
    flexDirection: 'row',
    height: 50,
    ...
    color: CommonColors.MortarColor, fontSize: SetFontSize.ts16, fontFamily: ConstantKeys.Averta_REGULAR
  },
  pickerTitleStyle: {
    justifyContent: 'center',
    flexDirection: 'row',

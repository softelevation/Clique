...
import {
    View, Text, SafeAreaView, StyleSheet, StatusBar, TouchableOpacity, Image, FlatList, LogBox,
...
export default class TempProfile extends Component {
                ...
                if (response.data.status == true) {
                    ...
                    for (var i = 0; i < social_data.length; i++) {
                        ...
                        if (item.media_value != '') {
                            ...
                            else if (item.media_type != 'externalLink' && item.media_type != 'payment' && item.media_type != 'music') {
                    ...
                    console.log("Filter payment : " + JSON.stringify(paymentSwitch.length))
    ...
    API_TEMP_ACTIVEINACTIVE(isload) {
        ...
        Webservice.post(APIURL.tempActiveInactive, {
            ...
            .then(response => {
                ...
                console.log('Get Temp Profile Active Inactive Response : ' + JSON.stringify(response))
    ...
    API_UPDATE_TEMP_PROFILE(isload, params) {
        ...
        Webservice.post(APIURL.tempProfileUpdate, {
            ...
            .then(response => {
                ...
                console.log('Get Temp Profile Update Response : ' + JSON.stringify(response))
    ...
    btnDoneTap = () => {
        requestAnimationFrame(() => {
            ...
            let websiteLinks = this.state.SocialData.filter(el => (
            ...
            let socialMailLinks = this.state.SocialData.filter(el => (
            ...
            let instagramLinks = this.state.SocialData.filter(el => (
            ...
            let facebookLinks = this.state.SocialData.filter(el => (
            ...
            let twitterLinks = this.state.SocialData.filter(el => (
            ...
            let youtubeLinks = this.state.SocialData.filter(el => (
            ...
            let linkdinLinks = this.state.SocialData.filter(el => (
            ...
            let homeNumberLinks = this.state.SocialData.filter(el => (
            ...
            let workNumberLinks = this.state.SocialData.filter(el => (
            ...
            let otherNumberLinks = this.state.SocialData.filter(el => (
            ...
            let musicLinks = this.state.MusicData.filter(el => (
                el.media_type === 'music'
            )).map((function ({ id, media_value, status }) {
                return { id, mediaValue: media_value, status };
            }))
            ...
            let paymentLinks = this.state.PaymentData.filter(el => (
            ...
            let externalLinks = this.state.ExternalLinkData.filter(el => (
    ...
    switchSocialItemUpdate = async (value, index) => {
        ...
        if (value) {
            ...
            let socialSwitch = this.state.SocialData.filter((item) => item.status == 0)
    ...
    switchPaymentItemUpdate = async (value, index) => {
        ...
        if (value) {
            ...
            let paymentSwitch = this.state.PaymentData.filter((item) => item.status == 0)
    ...
    switchExLinkItemUpdate = async (value, index) => {
        ...
        if (value) {
            ...
            let exLinkSwitch = this.state.ExternalLinkData.filter((item) => item.status == 0)
                            ...
                            {this.state.profileData != null ?
                                <ScrollView style={{ flex: 1, }} bounces={false}>
                                    ...
                                    <View style={{ width: '100%', height: 250 }}>
                                        ...
                                        <FastImage source={{ uri: String(this.state.userData.asset_url + this.state.profileData.avatar) }}
                                            style={{ flex: 1 }}
                                            resizeMode={FastImage.resizeMode.cover}
                                        />

                                        <View style={{ position: 'absolute', backgroundColor: CommonColors.transprentDark, bottom: 0, width: '100%', }}>
                                            <Text style={{
                                                marginLeft: 20, marginRight: 20, textAlign: 'left', marginTop: 15, fontSize: SetFontSize.ts25,
                                                color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_BOLD
                                            }}>
                                            ...
                                            {this.state.profileData.bio != '' ?
                                                <Text style={{
                                                    marginLeft: 20, marginRight: 20, textAlign: 'left', marginTop: 5, fontSize: SetFontSize.ts14,
                                                    color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR, marginBottom: 10
                                                }}>
                                                    {this.state.profileData.bio}
                                                </Text>
                                                : null}
                                    ...
                                    <View style={{ marginLeft: 20, marginRight: 20, marginTop: 20, flexDirection: 'row', alignItems: 'center', }}>
                                        <Text style={{
                                            flex: 1, fontSize: SetFontSize.ts18, marginRight: 5, color: CommonColors.whiteColor,
                                            fontFamily: ConstantKeys.Averta_BOLD
                                        }}>
                                        ...
                                        <Switch
                                            onValueChange={(value) => this.switchTempProfileUpdate(value)}
                                    ...
                                    <TouchableOpacity style={{
                                        ...
                                        onPress={() => Linking.openURL(`tel:${this.state.profileData.mobile}`)}>
                                    ...
                                    <TouchableOpacity style={{
                                        ...
                                        onPress={() => Linking.openURL(`mailto:${this.state.profileData.email}`)}>
                                        <Image style={{ marginLeft: 10, height: 30, width: 30, resizeMode: 'contain' }}
                                    ...
                                    {this.state.CompanyData.length != 0 ?
                                        <View style={{
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15, marginLeft: 15, marginRight: 15 }}>
                                            ...
                                            <View style={{ marginTop: 5 }}>
                                                <FlatList data={this.state.CompanyData}
                                                    keyExtractor={item => item.itemId}
                                                    renderItem={({ item, index }) => (

                                                        (item.job_position != '' ?
                                                            <TouchableOpacity style={{
                                                                alignItems: 'center', flexDirection: 'row', paddingBottom: 10, paddingTop: 10, marginTop: 10,
                                                                backgroundColor: CommonColors.appBarColor, borderRadius: 10, paddingLeft: 10, paddingRight: 10
                                                            }}
                                                                onPress={() => this.props.navigation.navigate('JobDetail', { data: JSON.stringify(item) })}>

                                                                <Image style={{ height: 50, width: 50, borderRadius: 25 }}
                                                                    source={{ uri: String(this.state.userData.asset_url + item.logo) }} />

                                                                <View style={{ flex: 1, justifyContent: 'center' }}>
                                                                    <Text style={{ flex: 1, marginLeft: 10, fontSize: SetFontSize.ts16, color: CommonColors.whiteColor, fontFamily: ConstantKeys.Averta_REGULAR }}
                                                                        numberOfLines={1}>
                                                                        {item.job_position}
                                                                    </Text>
                                    ...
                                    {this.state.SocialData.length != 0 ?
                                        <View style={{
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15, marginLeft: 15, }}>
                                                ...
                                                <Switch onValueChange={(value) => this.switchSocialUpdate(value)}
                                                    value={this.state.SocialSwitch == 1 ? true : false} />
                                            </View>

                                            <View style={{ marginTop: 5 }}>
                                                <FlatList data={this.state.SocialData}
                                                    keyExtractor={item => item.itemId}
                                                    scrollEnabled={false}
                                                    ...
                                                    renderItem={({ item, index }) => (
                                                        ...
                                                        (item.media_value != '' ?
                                                            <TouchableOpacity style={{
                                                                height: 50, alignItems: 'center', flexDirection: 'row', backgroundColor: CommonColors.appBarColor,
                                                                borderRadius: 10, marginTop: 10, paddingLeft: 10, paddingRight: 10
                                                            }}>

                                                                <Image style={{ height: 30, width: 30, borderRadius: 15, resizeMode: 'contain' }}
                                                                ...
                                                                <Switch
                                                                    onValueChange={(value) => this.switchSocialItemUpdate(value, index)}
                                    ...
                                    {this.state.MusicData.length != 0 ?
                                        <View style={{
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15, marginLeft: 15, }}>
                                                ...
                                                <Switch
                                                    onValueChange={(value) => this.switchMusicUpdate(value)}
                                                    value={this.state.MusicSwitch == 1 ? true : false} />
                                            </View>

                                            <View style={{ marginTop: 5 }}>
                                                <FlatList data={this.state.MusicData}
                                                    keyExtractor={item => item.itemId}
                                                    scrollEnabled={false}
                                                    extraData={this.state}
                                                    renderItem={({ item, index }) => (
                                                        ...
                                                        (item.media_value != '' ?
                                                            <TouchableOpacity style={{
                                                                height: 50, alignItems: 'center', flexDirection: 'row', backgroundColor: CommonColors.appBarColor,
                                                                borderRadius: 10, marginTop: 10, paddingRight: 10, paddingLeft: 10
                                                            }}>
                                                                ...
                                                                <Switch
                                                                    onValueChange={(value) => this.switchMusicItemUpdate(value, index)}
                                                                    value={item.status == 0 ? false : true} />
                                                            </TouchableOpacity>
                                                            : null)
                                                    )}
                                    ...
                                    {this.state.PaymentData.length != 0 ?
                                        <View style={{
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15, marginLeft: 15, }}>
                                                ...
                                                <Switch
                                                    onValueChange={(value) => this.switchPaymentUpdate(value)}
                                                    value={this.state.PaymentSwitch == 1 ? true : false} />
                                            </View>

                                            <View style={{ marginTop: 5 }}>
                                                <FlatList data={this.state.PaymentData}
                                                    keyExtractor={item => item.itemId}
                                                    scrollEnabled={false}
                                                    extraData={this.state}
                                                    renderItem={({ item, index }) => (
                                                        ...
                                                        (item.media_value != '' ?
                                                            <TouchableOpacity style={{
                                                                height: 50, alignItems: 'center', flexDirection: 'row', backgroundColor: CommonColors.appBarColor, borderRadius: 10,
                                                                ...
                                                                <Switch
                                                                    onValueChange={(value) => this.switchPaymentItemUpdate(value, index)}
                                                                    value={item.status == 0 ? false : true} />
                                                            </TouchableOpacity>
                                                            : null)
                                                    )}
                                    ...
                                    {this.state.ExternalLinkData.length != 0 ?
                                        <View style={{
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15, marginLeft: 15, }}>
                                                ...
                                                <Switch
                                                    onValueChange={(value) => this.switchExLinksUpdate(value)}
                                                    value={this.state.ExLinkSwitch == 1 ? true : false} />
                                            </View>

                                            <View style={{ marginTop: 5 }}>
                                                <FlatList data={this.state.ExternalLinkData}
                                                    keyExtractor={item => item.itemId}
                                                    scrollEnabled={false}
                                                    extraData={this.state}
                                                    renderItem={({ item, index }) => (
                                                        ...
                                                        (item.media_value != '' ?
                                                            <TouchableOpacity style={{
                                                                ...
                                                                <Switch
                                                                    onValueChange={(value) => this.switchExLinkItemUpdate(value, index)}
                                                                    value={item.status == 0 ? false : true} />
                                                            </TouchableOpacity>
                                                            : null)
                                                    )}
